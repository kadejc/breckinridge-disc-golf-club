...
**Bug found and fixed during the first (light-theme) redesign pass, worth remembering**: the
generic `button { color: #fff; ... }` rule (needed for the new gradient-fill buttons) silently
made `.nav-toggle` (the mobile hamburger, a bare `<button>`) render **white-on-white and
invisible**, because `.nav-toggle` overrides `background` but never declared its own `color` —
so the tag-level rule's text color leaked through even though `.nav-toggle`'s class selector has
higher specificity for the properties it *does* set. Fixed by adding an explicit
`color: var(--ink)` to `.nav-toggle` (now `var(--ink)` still resolves correctly under the dark
theme, since `--ink` was redefined to near-white). The general lesson: **any bare `<button>` (no
distinguishing class, or a class that doesn't set every property the generic `button` rule sets)
needs an explicit color check** after future button style changes — search for `<button` across
`site/**/*.html`, `shared/site-header.html`, and any JS that generates `<button>` markup
(currently none does; all JS-rendered tabs use `<div class="tab">`, not `<button>`).
...
## Homepage intro animation (2026-07-19, redesigned same day to POV + under 1s)

Original request: "a disc spinning into the basket and then it explodes into the site" — built as
a third-person shot (a small disc flew across the screen into a stationary basket, ~2.4s total).
**Immediately redone** per follow-up feedback: "Make it from the POV of the disc and make the
whole thing less than 1 second." This was a full concept change, not a tweak — worth knowing if
asked to touch this again, since the current version looks nothing like the first draft. Lives
entirely in `site/home.html` (markup + a page-scoped `<style>` block + a page-scoped `<script>`
block, all inline in that one file) — **homepage-only by design**, nothing shared with
`shared/site.css` or other pages, since an intro is inherently a one-time landing-page thing, not
a sitewide component. If asked to add it to other pages, that's a deliberate scope change to flag
back, not an oversight to "fix."

**How the POV version works**: there is no visible disc graphic anymore — POV means the camera
*is* the disc, so (like a first-person game not rendering the player's own body) the scene itself
rushes and spins instead of showing an object flying across it. One SVG (`viewBox="0 0 400 400"`)
contains: a basket (`#introBasket`, plain SVG shapes — pole, chain-support ellipse, six chain
lines, a basket-bin path) that animates `scale(.12)→scale(4.4)` combined with `rotate(0deg)→
rotate(900deg)` (2.5 spins) via one `translate(200px,200px) scale(...) rotate(...)` transform in
0.5s — this single transform is what sells "rushing toward and spinning into the basket," not a
translate-across-the-screen path like the old version; 12 radiating `<line>` "speed lines"
(`#introLines`, generated by a JS loop at 30° increments, same `--a` custom-property +
`rotate(var(--a)) translateX(...)` technique as the old version's burst shards, just retimed to
run *during* the rush instead of only at the end) that grow outward the whole time, doing double
duty as both "flying fast" motion lines during the rush and the "it explodes" burst at the end,
in one continuous 0.62s animation instead of two separate effects; and a flash `<circle>`
(`#introFlash`) that pulses r:0→220 with a 0.45s delay, timed to land right as the basket's rush
finishes. JS's only jobs are gating (below), building the 12 line elements once, adding `.play`
to start every keyframe together, and listening for the *overlay's own* fade-out `animationend`
to remove it from the DOM. **Full timing, verified via the Web Animations API (see below) — total
880ms, under the 1s ask with margin**: rush 0→500ms, speed lines 0→620ms, flash 450→770ms
(delayed to land at rush's end), overlay fade 580→880ms then `.remove()`d.
...
`position: fixed; inset: 0; z-index: 9999`, so for ~0.9s it covers (and blocks clicks on) the
entire page including the nav header — shorter than the original 2.4s version, but still enough
to break click-based tests if left unguarded. `tests/smoke.spec.js` has a `test.beforeEach` that calls
